...
(() => {
  ...
  const INTENTS = {
    ...
    lettering: {
      ...
      transparent: true, trim: true, aspect: '16:9'
    },
    ribbon: {
      label: 'Ribbon',
      takesText: true,
    ...
    paper: {
      label: 'Paper',
      build: (t, d) => `A flat sheet of ${d || 'lined writing paper'}, blank and unwritten. ` +
        'Scanned perfectly flat from directly above, filling the entire frame ' +
        'edge to edge with the paper surface only. ' +
        'No desk, no background, no shadow, no curl, no torn edge, no handwriting, ' +
        'no perspective, no objects on it.',
