/* ============================================================
   ...
   ============================================================ */
(() => {
  // Resolve relative to THIS script's own URL, so the app works at the
  // domain root, in a subfolder (GitHub Pages), or from file://.
  // A hardcoded '/libraries' 404s on every deploy that isn't at the root —
  // which is why the library tabs came up empty.
  const SELF = (document.currentScript && document.currentScript.src) || '';
  const BASE = SELF ? new URL('../libraries', SELF).href.replace(/\/$/, '') : 'libraries';
